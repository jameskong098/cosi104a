...
def train_ensemble(X_train, y_train, use_full_dataset):
    ...
    bagging_model = BaggingClassifier(estimator=base_estimator, n_estimators=50, random_state=42, n_jobs=-1)
    
    if use_full_dataset:
        # Option 1: Using Cross-validation for validation and the full training set for training
        cv_scores = cross_val_score(bagging_model, X_train, y_train, cv=5, scoring='f1_macro', n_jobs=-1)
        ...
        y_val_pred = bagging_model.predict(X_val_split)
